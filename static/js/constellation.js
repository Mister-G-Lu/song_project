...
const MODE_DESCRIPTIONS = {
    genre: 'Genre clusters — <span style="color:var(--text-secondary)">↑ favorites</span> float to top, <span style="color:var(--text-secondary)">↓ disliked</span> sink to bottom within each genre',
...
function _renderLegend(legendEl, data) {
    ...
    const genreHtml = genreEntries.length > 0
        ? `<span style="color:var(--text-muted);font-size:11px;margin-left:12px">↑ top ↓ bottom per genre: </span>`
          + genreEntries.map(([, meta]) =>
            `<span style="color:var(--text-muted);font-size:11px;margin-left:4px">${meta.dominant_genre}</span>`
          ).join('')
        : '';
...
function renderConstellation(data) {
    ...
    // Clear stale positions so new forces start from centre
    ...
    // Node colour — always sentiment-based in both modes
    const nodeColor = (d) => {
        if (currentMode === 'connections') {
    ...
    if (currentMode === 'connections') {
        ...
        // Genre & Taste: genre grid + vertical rating offset.

...
## ADR-005 — External rating imports are additive, never authoritative

**Date:** 2026-09-13
**Status:** Accepted

**Decision:** `scripts/import_rym_export.py` merges a RateYourMusic export into
`data/posts_tails.csv` under a strict "pre-existing wins" policy: an entry that
already exists keeps its rating no matter what the external source says, and
the only fields the importer may write into an existing row are empty ones
(`rating`, `artist`). It is dry-run by default; `--apply` is opt-in.

**Why:**
- Ratings in this project come from the owner's own reviews and are the training
  signal for the recommender. An older/differently-scaled number from another
  site is strictly worse evidence, so a conflict is never resolved in its
  favour. Per the request: "if I said 70 on website and 80 on rym, pick the
  initial 70".
- Conflicts are still *reported* (`report['conflicts']`) instead of being
  silently dropped, so the divergence can be reviewed song by song.
- Empty ratings are a real gap (455 rows here have a review but no number,
  mostly pre-CSV history). Filling them from the export is additive: it can't
  contradict anything.

**What the importer considers a duplicate:**
| tier | rule | effect |
|------|------|--------|
| `merge_sig` | `_normalize_sig(title)` equality — the same comparison `TasteEngine.deduplicate()`/`_merge_rows()` make | skip, always |
| `combo` | `_normalize_sig(artist + song)` | skip, always |
| `latin` | `_normalize_latin(...)` — CJK title vs its romanization | skip, always |
| `containment` / `jaccard` / `typo` | substring containment, Jaccard ≥ 0.95, `SequenceMatcher` ≥ 0.90, all artist-gated | skipped and listed under `needs_review`; `--fuzzy add` imports them |

Tier 1 is mandatory rather than merely tidy: the overlay merge keeps the **higher**
rating, so a row whose normalized title collides with an existing one would win
against the original review if it were allowed in with a bigger number. Being
*stricter* than the engine (tiers 2–6) is safe in the other direction — a near
duplicate in the base CSV is never merged away by anyone, so it has to be
prevented on the way in.

**Scale handling:** RYM ratings are 0.5–5.0 stars, this project's are 0–100.
The exporter's scale is detected from the column (all values ≤ 5 → stars, ×20)
and overridable with `--scale anchored|linear|out_of_10|as_is`; letter grades
reuse `data/backfill_data.json` so an imported `B+` means the same 88 the
backfiller already implies.

**Alternative considered:** extend `/api/import-songs` (pasted text). Rejected:
that path appends to the overlay with no duplicate check at all, and an
800-row export is not something to paste; a script can be re-run, audited in
git, and keep a machine-readable report.

**Consequence:** `gap_metrics()` in the same script reports coverage before and
after (rated %, artist %, release-year %, review-text %), which is how "which
gaps did this export cover" gets answered with numbers instead of vibes.

...
class TestExtractSearchTerms:
    ...
    def test_extract_empty_title(self):
        """Handle empty title."""
        helper = SpotifyHelper()
        result = helper.extract_search_terms({'title': ''})
        assert result['song'] == ''

    def test_extract_no_title_key(self):
        """Handle missing title key."""
        helper = SpotifyHelper()
        result = helper.extract_search_terms({})
        assert result['song'] == ''


class TestSpotifyEdgeCases:
    """Test edge cases in Spotify helper."""
    ...
    def test_recommendations_no_seeds_edge(self):
        """Empty recommendations with no seeds or init."""
        os.environ.pop('SPOTIFY_CLIENT_ID', None)
        os.environ.pop('SPOTIFY_CLIENT_SECRET', None)
        helper = SpotifyHelper()
        result = helper.get_recommendations_from_seeds()
    ...
    def test_create_playlist_returns_none(self):
        """create_playlist should return None (placeholder until user auth)."""
        os.environ.pop('SPOTIFY_CLIENT_ID', None)
        os.environ.pop('SPOTIFY_CLIENT_SECRET', None)
        helper = SpotifyHelper()
        result = helper.create_playlist('user_id', 'name')
        assert result is None

    def test_extract_search_terms_dash_with_parenthetical(self):
        """Extract from 'Artist - Song (Extra)' format."""
        helper = SpotifyHelper()
        result = helper.extract_search_terms({'title': 'Artist – Song (Remix, 2024)'})
        assert result['artist'] == 'Artist'
        assert result['song'] == 'Song'

    def test_extract_search_terms_only_artist_dash(self):
        """Extract from 'Artist - Song' without year."""
        helper = SpotifyHelper()
        result = helper.extract_search_terms({'title': 'Lindsey Stirling – Crystallize'})
        assert result['artist'] == 'Lindsey Stirling'
        assert result['song'] == 'Crystallize'

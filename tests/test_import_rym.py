...
@pytest.mark.parametrize('artist', ['LOONA 1/3', 'Aly & AJ',
                                   'ABBA (Björn, Benny, Agnetha & Frida)'])
def test_import_preserves_full_artist_credit(artist):
    cand = ImportRow({'artist': 'artist', 'song': 'song'},
                     {'artist': artist, 'song': 'Example'})
    assert cand.artist == artist
    assert cand.title == f'Example ({artist})'
